...
**confidence 점수 기준 (엄격하게 따르세요):**
- 0.1~0.3: 약한 징후. 콘텐츠에 해당 본능/편향의 흔적이 있지만 명확하지 않음.
- 0.4~0.6: 중간 수준. 콘텐츠에서 해당 패턴이 분명히 관찰되나, 의도적이라고 단정하기 어려움.
- 0.7~0.9: 강한 징후. 콘텐츠가 해당 본능/편향을 명백하게 활용하고 있으며, 구체적 증거가 다수 존재.
- **대부분의 항목은 0.2~0.5 범위에 있어야 합니다. 0.7 이상은 정말 명백한 경우에만 부여하세요.**
- **콘텐츠에서 해당 본능/편향의 직접적 증거가 없으면 리스트에 포함하지 마세요.**
- 10가지 본능과 5가지 편향 전부를 포함할 필요 없습니다. 실제로 감지된 것만 포함하세요 (보통 각각 2~4개).
...
다음 JSON 구조로 분석 결과를 출력하세요:
{
 ...
 "user_instincts": [
   ...
   },
   {
     "instinct_type": "간극 본능",
     "confidence": 0.72,
     "reasoning": "명백한 이분법적 구도가 사용된 이유 설명",
     "example": "콘텐츠에서의 구체적 사례"
   }
 ],
 "information_biases": [
...
당신의 임무는 동일한 주제에 대한 대안적 관점을 찾고, 서로 다른 소스들이 동일한 사실을 어떻게 프레임화하는지 **상세하게** 분석하는 것입니다.
...
2. **각 관점에 대해 깊이 있는 분석을 수행하십시오:**
   - main_claim: **2-3문장**으로 주요 논거를 상세히 기술 (단순 요약 금지)
   - main_claim_reasoning: 그 논거가 왜 설득력이 있는지 논증 근거 설명
   - frame_type: 프레임 유형 분류 (economic/social/political/ethical/scientific/emotional/historical/other)
   - frame_description: 해당 프레임이 어떻게 이슈를 구성하는지 상세 설명 (어떤 측면을 강조하고 배제하는지)
   - key_points: **5개**의 핵심 포인트 (각각 구체적으로)
   - key_point_details: 각 포인트에 대한 상세 설명과 뒷받침 데이터
   - evidence: 근거 자료 (통계, 인용문, 연구, 사례, 전문가 의견 중 2-3개)
   - methodology: 이 관점이 문제에 접근하는 방법론 설명
3. **의제의 성격에 따라 스펙트럼의 유형을 결정하십시오:**
   - **'선형 (linear)'**: 이슈가 하나의 축(예: 찬성 vs 반대)을 따라 양극단 사이의 위치로 설명될 때.
   - **'발산형 (divergent)'**: 공통된 사실에서 시작하여 여러 관점이 각기 다른 방향으로 흩어질 때.
   - **'차원적 (dimensional)'**: 이슈가 두 가지 이상의 독립적인 축으로 복합적으로 구성될 때.
4. 각 관점을 해당 스펙트럼 수치에 매핑하십시오.
5. 모든 관점이 동의하는 사실(공통 분모)을 식별하십시오.
6. 핵심적인 견해 차이 지점을 식별하십시오.
7. **가장 적합한 시각화 유형을 결정하십시오.**
...
다음 JSON 구조로 결과를 출력하십시오:
{
 "perspectives": [
   {
     ...
     "source": {
       ...
       "title": "기사/콘텐츠 제목",
       "publisher": "발행처",
       "published_date": "발행일 (YYYY-MM-DD 또는 '날짜 불명')",
       "credibility_score": 0.8
     },
     "main_claim": "주요 논거를 2-3문장으로 상세히 기술. 이 관점이 주장하는 핵심 내용과 그 맥락을 포함해야 함. 단순한 한 줄 요약이 아닌 충분한 설명을 제공.",
     "main_claim_reasoning": "위 주장이 왜 설득력이 있는지, 어떤 논리 구조와 전제를 기반으로 하는지 설명. 이 관점을 지지하는 사람들의 핵심 논거.",
     "frame": "프레임 태그 (예: 경제적 효율성, 사회적 형평성, 안보 관점)",
     "frame_type": "economic|social|political|ethical|scientific|emotional|historical|other",
     "frame_description": "이 프레임이 어떻게 이슈를 구성하는지 상세히 설명. 어떤 측면을 강조하고, 어떤 측면을 배제하거나 축소하는지 분석.",
     "key_points": [
       "핵심 포인트 1: 구체적인 주장",
       "핵심 포인트 2: 구체적인 주장",
       "핵심 포인트 3: 구체적인 주장",
     ...
     "key_point_details": [
       {
         "point": "핵심 포인트 1",
         "explanation": "이 포인트가 의미하는 바와 왜 중요한지 상세 설명",
         "supporting_data": "뒷받침하는 구체적 데이터, 사례, 또는 인용 (있는 경우)"
       }
     ],
     "evidence": [
       {
         "type": "statistic|quote|study|example|expert_opinion",
         "content": "근거 내용 (구체적인 숫자, 인용문, 연구 결과 등)",
         "source": "출처 (있는 경우)",
         "reliability": "high|medium|low"
       }
     ],
     "methodology": "이 관점이 문제에 접근하는 방법론. 예: 데이터 기반 실증 분석, 역사적 비교, 원칙 기반 연역적 논증, 사례 연구 등",
     "spectrum": {
       "political": 0.5,
       "emotional": -0.2,
       "complexity": 0.3
 ...
 "common_facts": ["모든 관점이 동의하는 사실 1", "모든 관점이 동의하는 사실 2"],
...
분석 결과를 바탕으로 사용자의 사고를 확장할 수 있는 **구체적이고 맥락화된** 대안적 프레이밍과 연관 주제를 생성하세요.
...
1. **대안적 프레이밍 (Alternative Framing)** - 가장 중요!
   이 콘텐츠가 사용하는 프레임의 한계를 지적하고, 동일한 사실/이슈를 완전히 다른 관점에서 해석하는 방법을 **구체적으로** 제시하세요.

   반드시 포함해야 할 요소:
   - 현재 프레임이 강조하는 것과 누락하는 것을 명시
   - 반대 관점에서 볼 때 어떤 요소가 다르게 해석되는지 구체적 예시
   - 제3의 프레임(양쪽 모두와 다른 시각)이 있다면 언급
   - 독자가 스스로 생각해볼 수 있는 핵심 질문 1개

   **4-6문장**으로 상세하게 작성하세요. 일반적인 조언이 아닌, 이 특정 콘텐츠에 맞춤화된 분석이어야 합니다.
...
출력 JSON:
{{
  "alternativeFraming": "현재 콘텐츠가 [구체적 프레임]의 관점에서 작성되어 [강조하는 측면]에 초점을 맞추고 있습니다. 그러나 [반대 관점]에서 보면 [다르게 해석되는 요소]가 있습니다. 예를 들어 [구체적 예시]. 또한 [제3의 관점]에서는 [다른 해석]이 가능합니다. 스스로 생각해볼 질문: [핵심 질문]?",
  "expandedTopics": [
    {{
      "topic": "확장 주제 이름",
      "description": "이 주제가 현재 이슈와 어떻게 연결되는지 구체적 설명",
      "relevance": "high"
    }},
    {{
      "topic": "확장 주제 이름",
